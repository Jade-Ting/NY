<template>
  <div class="wrap">
    ...
    <div class="block"></div>
    <div class="thingbox" v-show="show" v-for="(item,index) in fabuinfo" :key="index">
      <div class="username">
        <img :src="url" alt>
      ...
      <div class="swiper-box">
        <van-swipe :options="swiperOption">
          <van-swipe-item class="show" v-for="(date,index) in item.imgUrls" :key="index">
      ...
      <div class="operate">
        <div class="del" v-if="item" @click="delorder(item)">删除发布</div>
      </div>
    </div>
...
export default {
  ...
  mounted() {
    this.url = `../${window.sessionStorage.getItem("useravatar")}`;
    this.username = window.sessionStorage.getItem("username");
    this.fabuinfo = this.$store.getters.fabuinfo;
    this.noFabu();
  },
  methods: {
    back() {
    ...
    delorder(e) {
      Dialog.confirm({ message: "确定要删除你的发布吗？" }).then(action => {
